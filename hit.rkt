...
(define (flip-parity hit)
  (match hit
    [(Hit 'IN t pt n mat) (Hit 'OUT t pt (fl3-negate n) mat)]
    [(Hit 'OUT t pt n mat) (Hit 'IN t pt (fl3-negate n) mat)]))

(: get-emission : Ray Hit -> RGB)
;; given a ray and a hit record for it, get the emission rgb value
;; info for the hit record's material
(define (get-emission ray hit)
  (match hit
    [(Hit _ _ _ norm (Material emit _)) (emit ray norm)]))
...
(define (first-entry hits)
  (match hits
    [(cons hit hitr) (if (and (not (= (Hit-t hit) +inf.0)) (entry? hit))
                         (Some hit)
                         (first-entry hitr))]
