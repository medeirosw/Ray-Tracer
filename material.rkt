...
(define (lambertian-material aten)
  (Material no-emission
            (lambda ([ray : Ray] [pt : Float3] [norm : Float3])
              (Some (Reflect-Info aten (make-ray pt (fl3+ norm (random-point-in-sphere))))))))
...
(define (metal-material aten fuzz)
  (Material no-emission
            (lambda ([ray : Ray] [pt : Float3] [norm : Float3])
              (local
                {(define s : Float3 (fl3-scale+ (fl3-reflect (Ray-dir ray) norm)
                                                fuzz
                                                (random-point-in-sphere)))}
                (if (<= (fl3-dot s norm) 0.0) 'None (Some (Reflect-Info aten (Ray pt s))))))))

(: diffuse-emissive-material : RGB -> Material)
;; returns specificed rgb value for the material's emit function
(define (diffuse-emissive-material rgb)
  (local
    {(: emit : Emit-Fun)
     ;; emit function for material
     (define (emit ray norm) rgb)
     (: none-mat : Scatter-Fun)
     ;; scatter function for material
     (define (none-mat ray intersect norm) 'None)}
    (Material emit none-mat)))
